...
class USBClient:
    ...
    def _do_home(self) -> None:
        """Envia o comando de homing (SET_HOME_CMD) ao robo."""
        robo = self._robo
        msg = Message()
    ...
    def _do_send(self, method: str, params: dict[str, Any]) -> Any:
        ...
        if method == "set_homecmd":
            ...
            # ptp_mode: 1 = MOVJ (por articulacao), 2 = MOVL (linear)
            mode = params.get("ptp_mode", 1)
            ptp = PTPMode.MOVL_XYZ if mode == 2 else PTPMode.MOVJ_XYZ
            robo._set_ptp_cmd(
                x=params.get("x", 0),
                y=params.get("y", 0),
                z=params.get("z", 0),
            ...
            robo.speed(params.get("planVel", 100), params.get("planAcc", 100))
            return None
